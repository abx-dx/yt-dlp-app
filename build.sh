...
PORTABLE_CLI_DIR="$DIST_DIR/yt-dlp-cli"
...
echo "========================================================"
echo "🚀 yt-dlp-cli PORTABLE BUILD"
echo "========================================================"
...
if [ ! -f "$TARGET_PROJECT_DIR/cli.py" ]; then

    echo "❌ cli.py bulunamadı:"
    echo "   $TARGET_PROJECT_DIR/cli.py"

    exit 1

fi

if [ ! -d "$TARGET_PROJECT_DIR/terminal" ]; then

    echo "❌ terminal klasörü bulunamadı:"
    echo "   $TARGET_PROJECT_DIR/terminal"
...
do
    ...
    if [ ! -f "$TARGET_PROJECT_DIR/terminal/$file" ]; then

        echo "❌ terminal/$file bulunamadı:"
        echo "   $TARGET_PROJECT_DIR/terminal/$file"
...
CLI_REQUIREMENTS="$BUILD_DIR/cli-requirements.txt"
...
echo "   → CLI requirements hazırlanıyor..."
...
echo "[6/8] CLI + terminal + core hazırlanıyor..."

# ------------------------------------------------------------------------------
# CLI
# ------------------------------------------------------------------------------

mkdir -p "$PORTABLE_CLI_DIR"

cp \
    "$TARGET_PROJECT_DIR/cli.py" \
    "$PORTABLE_CLI_DIR/cli.py"
...
find "$TARGET_PROJECT_DIR/terminal" \
    -type f \
    -name '*.py' \
    -print0 |
while IFS= read -r -d '' SOURCE_FILE; do

    RELATIVE_FILE="${SOURCE_FILE#"$TARGET_PROJECT_DIR/"}"
    TARGET_FILE="$PORTABLE_CLI_DIR/$RELATIVE_FILE"
...
if [ ! -f "$PORTABLE_CLI_DIR/terminal/__init__.py" ]; then

    echo "❌ Portable terminal paketi oluşturulamadı."

    exit 1

fi

echo "   ✅ terminal paketi hazır."
...
"%~dp0yt-dlp-python\python.exe" "%~dp0yt-dlp-cli\cli.py"
CMD
...
if [ ! -f "$PORTABLE_CLI_DIR/cli.py" ]; then

    echo "❌ Portable cli.py bulunamadı."
...
if [ ! -f "$PORTABLE_CLI_DIR/terminal/__init__.py" ]; then
    ...
    echo "❌ Portable terminal paketi bulunamadı."
...
echo "   → Python/Core import kontrolü çalıştırılıyor..."

if ! "$PYTHON_EXE" -c "
import sys
from pathlib import Path

cli_dir = Path(r'$PORTABLE_CLI_DIR')
core_dir = Path(r'$PORTABLE_CORE_DIR')
...
import terminal.download
import terminal.console

import toolbox
import toolbox.command
import toolbox.cookies
import toolbox.metadata
import toolbox.output
import toolbox.parser
import toolbox.playlist
import toolbox.playlist_info
import toolbox.profiles
import toolbox.runner
import toolbox.tools
import toolbox.resolver
"; then

    echo "❌ Python CLI/Core import kontrolü başarısız."

    exit 1

fi

echo "   ✅ CLI/Core import kontrolü başarılı."

echo "   → Python syntax kontrolü çalıştırılıyor..."

if ! "$PYTHON_EXE" -m py_compile \
    "$PORTABLE_CLI_DIR/cli.py" \
    "$PORTABLE_CLI_DIR/terminal"/*.py \
    "$PORTABLE_CORE_DIR/toolbox"/*.py; then

    echo "❌ Python syntax kontrolü başarısız."

    exit 1

fi

echo "   ✅ Python syntax kontrolü başarılı."

# ------------------------------------------------------------------------------
# NODRIVER KONTROLLERİ
# ------------------------------------------------------------------------------
...
echo "   ✅ yt-dlp-cli:"
echo "      $PORTABLE_CLI_DIR"

echo "   ✅ terminal:"
echo "      $PORTABLE_CLI_DIR/terminal"
