...
PORTABLE_GUI_DIR="$DIST_DIR/yt-dlp-gui"
...
echo "========================================================"
echo "🚀 yt-dlp-gui PORTABLE BUILD"
echo "========================================================"
...
if [ ! -f "$TARGET_PROJECT_DIR/gui.py" ]; then
    ...
    echo "   $TARGET_PROJECT_DIR/gui.py"
...
echo "[6/8] GUI ve core hazırlanıyor..."
...
"%~dp0yt-dlp-python\python.exe" "%~dp0yt-dlp-gui\gui.py"
CMD
...
if [ ! -f "$PORTABLE_GUI_DIR/gui.py" ]; then
    echo "❌ Portable gui.py bulunamadı."
    exit 1
fi
...
"; then
    ...
    echo "❌ Python paket/Core import kontrolü başarısız."

    exit 1

fi

echo "   → GUI self-test çalıştırılıyor..."

if ! "$PYTHON_EXE" \
    "$PORTABLE_GUI_DIR/gui.py" \
    --self-test; then

    echo "❌ GUI self-test başarısız."
